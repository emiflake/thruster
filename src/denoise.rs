...
impl Denoiser {
    ...
    fn create_output_image((w, h): (u32, u32), out_vec: Vec<f32>) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
        let mut buf = ImageBuffer::<Rgba<u8>,Vec<u8>>::new(w, h);

        for y in 0..h {
            for x in 0..w {
                let mut p = buf.get_pixel_mut(x, y);
                for c in 0..3 {
    ...
    pub fn denoise(&self, image_buf: ImageBuffer<Rgba<u8>, Vec<u8>>) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
